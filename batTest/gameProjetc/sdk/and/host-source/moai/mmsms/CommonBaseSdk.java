...
public static void getCopInfo(){
	...
	configFileName=String.format("cConfig_%d.json", 1);
	...
	if(CommonTool.cardType==CommonTool.CardType_YD){//移动
		...
		else {configFileName=String.format("cConfig_%d.json", 1);}
		getConfig();
...
public static void initSdk(){
	...
	if (CommonTool.cardType==CommonTool.CardType_YD && CommonTool.sdkId==1){configFileName=String.format("cConfig_%d.json", 1);cardType=1;}
	...
	if (CommonTool.cardType==-1){
		...
		else {configFileName=String.format("cConfig_%d.json", 1);cardType=1;}
		
	}
	
	getConfig();// 不要和 sActivity=activity; 交换位置
	
	CommonMMSMS.ResultChannelInfo("");
