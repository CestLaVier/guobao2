...
public class CommonAnd  extends  CommonBaseSdk {
	...
	public  static Callbacklister _listener =null;
    ...
    public static void setBillingConfig(){
        ...
        if(billingMap!=null){
            ...
            for(int i=0;i<billingMap.size();i++){
                ...
                String nametring="";
                try {
                	Log.e("wx","不是utf8汉字-->"+CommonBaseSdk.GetJsonVal(mapJson,"name","0"));
					nametring = new String(CommonBaseSdk.GetJsonVal(mapJson,"name","0").getBytes("UTF-8"));
					Log.e("wx","utf8汉字-->"+nametring);
...
//是否退出时执行
    ...
    { 
        try { 
             ...
             WeChatPay.proName=bConfig.name;
             WeChatPay.proPrice=String.valueOf(bConfig.money);
             
             Intent intent = new Intent(sActivity, OpenpayDalog.class);
     	      
            intent.putExtra("payCode", 			payCode);
         	intent.putExtra("price", 				String.valueOf((float)bConfig.money/100));
         	intent.putExtra("proName", 				bConfig.name);
		    ...
		    WXPayEntryActivity._listener = _listener;
