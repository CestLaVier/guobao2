...
public static void getCopInfo(){
	...
	configFileName=String.format("cConfig_%d.json", 2);
	Log.e("sdk","getCopInfo");
	if(CommonTool.cardType==CommonTool.CardType_YD){//移动
		if(isMM) {configFileName=String.format("cConfig_%d.json",2);}
		else {configFileName=String.format("cConfig_%d.json", 2);}
		getConfig();
...
public static void initSdk(){
	...
	if (CommonTool.cardType==CommonTool.CardType_YD && CommonTool.sdkId==1){configFileName=String.format("cConfig_%d.json", 2);cardType=1;}
	if (CommonTool.cardType==CommonTool.CardType_YD && CommonTool.sdkId==2){configFileName=String.format("cConfig_%d.json", 2);cardType=1;}

	if (CommonTool.cardType==-1){
		Log.e("sdk","fuck"+String.valueOf(isMM));
		if (isMM == true){configFileName=String.format("cConfig_%d.json", 2);cardType=1;}
		else {configFileName=String.format("cConfig_%d.json", 2);cardType=1;}
		
	}
	
	getConfig();// 不要和 sActivity=activity; 交换位置
	
	CommonUnipay.ResultChannelInfo("");
